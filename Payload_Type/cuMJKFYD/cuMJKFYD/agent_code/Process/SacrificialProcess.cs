...
using AI = YXBqSVSFInterop.Classes.Core;
...
namespace Process
{
    public class SacrificialProcess : AI.Process
    {
        ...
        public override bool StartWithCredentials(YXBqSVSFLogonInformation logonInfo)
        {
            bool bRet = false;
        ...
        public override bool StartWithCredentials(IntPtr hToken)
        {
            ...
            else
            {
                ...
                if (!bRet && (dwError == 1314)) // ERROR_PRIVILEGE_NOT_HELD or FILE_NOT_FOUND
                {
                    ...
                    {
                        if (_agent.GetIdentityManager().GetCurrentLogonInformation(out YXBqSVSFLogonInformation cred))
                        {
