...
A fully featured .NET 4.0 compatible training agent. Version: {}
    ...
    agent_path = pathlib.Path(".") / "cuMJKFYD" / "mythic"
    agent_code_path = pathlib.Path(".") / "cuMJKFYD" / "agent_code"
    agent_icon_path = agent_path / "agent_functions" / "cuMJKFYD.svg"
    build_steps = [
    ...
    async def build(self) -> BuildResponse:
        ...
        success_message = f"YXBqSVSF {self.uuid} Successfully Built"
        stdout_err = ""
        ...
        try:
            ...
            if stderr:
                stdout_err += f'[stderr]\n{stderr.decode()}' + "\n" + command
            output_path = "{}/YXBqSVSF/bin/Release/YXBqSVSF.exe".format(agent_build_path.name)
            ...
            if os.path.exists(output_path):
                ...
                targetInteropPath = "/srv/YXBqSVSFInterop.dll"
                ...
                shutil.move("{}/YXBqSVSFInterop/bin/Release/YXBqSVSFInterop.dll".format(agent_build_path.name), targetInteropPath)
